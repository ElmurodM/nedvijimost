<?php
/**
 * Customizer Control: real-home-repeater
 *
 * @package Real_Home
 */
 ...
 * Repeater control
 */
class Real_Home_Customize_Repeater_Control extends WP_Customize_Control {
     ...
     * @var string
     */
    public $type = 'real-home-repeater';
    ...
    public function __construct( $manager, $id, $args = array() ) {
        ...
        $this->row_label = array(
            'type' => 'text',
            'value' => esc_attr__( 'row', 'real-home' ),
            'field' => false,
        );
        ...
        if ( empty( $this->button_label ) ) {
            $this->button_label = sprintf( __( 'Add new %s', 'real-home' ), esc_html( $this->row_label['value'] ) );
        }
    ...
    public function enqueue() {
        ...
        wp_enqueue_script( 'real-home-repeater', REAL_HOME_THEME_URI . 'inc/customizer/controls/repeater/assets/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
        wp_enqueue_style( 'real-home-repeater', REAL_HOME_THEME_URI . 'inc/customizer/controls/repeater/assets/repeater.css', null );

        wp_localize_script( 'real-home-repeater', 'real_home_customize',
            array(
                'nonce' => wp_create_nonce( 'real_home_customize_nonce' )
            )
        );
    }
    ...
    protected function render_content() {
        ...
        <?php if ( isset( $this->choices['limit'] ) ) : ?>
            <p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'real-home' ), esc_html( $this->choices['limit'] ) ); ?></p>
        <?php endif; ?>
                ...
                <div class="repeater-row-content">
                    ...
                    <div class="repeater-field repeater-field-{{{ field.type }}}">
                        ...
                        <label>
                            ...
                            <span class="description customize-control-description">{{{ field.description }}}</span>
                            <# } #>
                            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'real-home' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
                        ...
                        <figure class="real-home-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'real-home' ); ?>" >
                            <# if ( field.default ) { #>
                            <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                            <img src="{{{ defaultImageURL }}}">
                            <# } else { #>
                            <?php esc_attr_e( 'No Image Selected', 'real-home' ); ?>
                            <# } #>
                        </figure>

                        <div class="actions">
                            <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'real-home' ); ?></button>
                            <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'real-home' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'real-home' ); ?>" >
                                <# if ( field.default ) { #>
                                <?php esc_attr_e( 'Change Image', 'real-home' ); ?>
                                <# } else { #>
                                <?php esc_attr_e( 'Add Image', 'real-home' ); ?>
                                <# } #>
                            </button>
                            <# if ( field.default.id ) { #>
                        ...
                        <figure class="real-home-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'real-home' ); ?>" >
                            <# if ( field.default ) { #>
                            ...
                            <# } else { #>
                            <?php esc_attr_e( 'No File Selected', 'real-home' ); ?>
                            <# } #>
                        </figure>

                        <div class="actions">
                            <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                            <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'real-home' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'real-home' ); ?>" >
                                <# if ( field.default ) { #>
                                <?php esc_attr_e( 'Change File', 'real-home' ); ?>
                                <# } else { #>
                                <?php esc_attr_e( 'Add File', 'real-home' ); ?>
                                <# } #>
                            </button>
                            <# if ( field.default.id ) { #>
                            <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
                    ...
                    <# }); #>
                    <button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'real-home' ); ?></button>
                </div>
            </li>
        </script>
        <?php
